...
const post = async ({ fullname, birthday, contact, car_number, car_name }) => {
    try {
        ...
        const result = await db(true, POST_DRIVER, fullname, birthday, contact, car_number, car_name)
        return result 
    }
    catch (error) {
        throw error
...
const update = async ({ id, fullname, birthday, contact, car_number, car_name, status }) => {
    try {
        const UPDATE_DRIVER = `
            with old_data as (
                select 
        ...
        const result = await db(true, UPDATE_DRIVER, id, fullname, birthday, contact, car_number, car_name, status)
        return result
    }
    catch (error) {
        throw error
